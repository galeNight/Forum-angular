...
export class ProfileService {
  ...
  deleteAccount(accountID:number):Observable<profileResponse[]>{
    const headers ={'content-type':'application/json'}
    let data = this.http.delete<profileResponse[]>(this.apiurl+"DeleteAccount/"+"?id="+accountID/*match with backend controller endpoint HttpName*/,{'headers':headers})
    console.log(data);
    return data;
}
}